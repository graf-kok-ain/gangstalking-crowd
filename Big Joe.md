# Big Joe

## Basic Information
- **Spotify ID**: 4GesfwNcRZ8bCDyHdGcbsD
- **Spotify Link**: https://open.spotify.com/artist/4GesfwNcRZ8bCDyHdGcbsD
- **Original Spotify-Link**: https://open.spotify.com/artist/4GesfwNcRZ8bCDyHdGcbsD?si=f6523f81492b4cdc
- **Collection Timestamp**: 26.02.2026 02:12 (MCP Browser Tool - LATEST)
- **Updated Timestamp**: 28.02.2026 20:20 (Page Loading Issues Confirmed via MCP Browser)
- **Monthly Listeners**: 4 (captured: 26.02.2026 02:12)
- **Followers**: Not available
- **Genre**: Rap/Trap
- **Popularity**: Extremely low (4 monthly listeners)

## Biography
"#UNDERDAWGYEAR"

## Online Presence
- **Official Website**: Not found
- **Wikipedia**: No article available
- **Instagram**: Not found
- **Twitter**: Not found
- **Other Social Media**: Not found

## Archive.is
- **Archive Link**: CAPTCHA blocker requires manual capture
- **Archive Date**: 26.02.2026 02:12

## Discography
### Albums
- No albums available

### Singles
- Party Is Over (2021) - Single

### Popular Tracks (Top 1)
1. Party Is Over (feat. pestotrip) (Explicit) - 1,134 Plays

## Collaborations
- pestotrip
- toe
- utpv
- CONTRITUM
- Never Sober
- rookie白冉
- Kid Pullen

## Featured On
- No Smoke (2022) - Single

## Patterns & Anomalies
- **Name Conventions**: English name "Big Joe" with hashtag bio
- **Themes**: Party culture, underdog mentality ("#UNDERDAWGYEAR")
- **Language Patterns**: English with trap elements
- **Number Mysticism**: No obvious number patterns

## Manipulation Indicators
- **Listener Count Anomalies**: 4 monthly listeners with 1,134 plays (extremely suspicious)
- **Activity Patterns**: Minimal content, but high play counts
- **Bot Indicators**: High play counts with almost non-existent listeners strongly indicates bot activity
- **Artificial Popularity**: Extremely suspicious ratio between plays and listeners

## Network Analysis
- **Collaboration Density**: Moderate collaborations with similar low-count artists
- **Genre Cluster**: Trap with party themes
- **Connection Strength**: Weak connections, but part of "Fans also like" cluster with Never Sober and Kid Pullen
- **Influence Patterns**: Minimal influence, but potential bot cluster formation

## Status
✅ 100% fully captured (Archive.is to be completed manually)
